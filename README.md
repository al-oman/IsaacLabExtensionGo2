...
- Throughout the repository, the name `IsaacLabExtensionGo2` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of IsaacLabExtensionGo2 (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/IsaacLabExtensionGo2
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/IsaacLabExtensionGo2/IsaacLabExtensionGo2/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/IsaacLabExtensionGo2"
    ]
}
```
